...
Commit messages use [Conventional Commits](https://www.conventionalcommits.org/), enforced by commitlint via the husky `commit-msg` hook. After `npm install`, the prepare script installs husky locally (skipped when `CI=true` or `HUSKY=0`).
...
Current app version: **2.0.0** (root and workspace `package.json` fields must stay in sync).
